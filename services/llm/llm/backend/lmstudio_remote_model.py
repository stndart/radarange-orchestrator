...
from ..core.llm_backend import LLM_Config
from ..tools import Tool
from .generic_model import GenericModel
from .lmstudio_bindings import (
    from_lms_message,
    from_lms_response,
    to_lms_chat,
    to_lms_tools,
...
class LMSModel(GenericModel):
    ...
    def __init__(self, host: str, model: str, config: Optional[LMSConfig] = None):
        ...
        self.model = self.client.llm.model(
            self.model_id, ttl=self.default_ttl, config=self.config
        )
    ...
    def assure_loaded(self) -> None:
        loaded = [
            m.identifier for m in self.client._get_session(SyncSessionLlm).list_loaded()
        ]
    ...
    def create_chat_completion(
        ...
        response_format_json = (
            None if response_format is None else response_format.json_schema
        )
        ...
        if stream:
            ...
            if len(all_tools) > 0:
                raise NotImplementedError(
                    'Tool support for lms.llm.respond is not implemented'
                )
    ...
    def act(
        ...
        def on_message_handler(message: lms.AssistantResponse | lms.ToolResultMessage):
            if isinstance(message, lms.AssistantResponse):
                ...
                raise NotImplementedError(
                    'Critical error: on_message handler received something not assistant response or tool call response'
                )
