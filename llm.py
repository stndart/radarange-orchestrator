...
from .llm_types import LlmConfig, ChatMessage, Response, ResponseStream, MessageHandler, EmptyMessageHandler
from .llama_cpp_bindings import to_llama_tools, from_chat_completion
from .chat import Chat

class llm:
    def __init__(self, model: str = "QwQ-32B-Q4_K_M.gguf",
    ...
    def chat(self, system_prompt: str = "", tools: list[Callable[..., Any]] | list[Tool] = []) -> Chat:
        return Chat(ChatMessage(role='system', content=system_prompt), tools=tools)
    
    def respond_stream(self, prompt: Chat, temperature: float = 0.7, max_tokens: int = -1) -> ResponseStream:
        raise NotImplementedError("Stream support is not yet implemented")
    
    def respond(self, prompt: Chat | str, tools: list[ToolHandler] | list[Tool] = [], temperature: float = 0.7, max_tokens: int = -1, response_format: Optional[str] = None) -> Response:
        if isinstance(prompt, str):
            return self.respond(Chat(prompt), tools = tools, temperature = temperature, max_tokens = max_tokens, response_format = response_format)
        else:
            full_tools: list[Tool] = prompt.tools + [fun if isinstance(fun, Tool) else make_tool_from_fun(fun) for fun in tools]
            chat_completion = self.llm.create_chat_completion(
                messages=prompt.llama_messages(),
                tools=to_llama_tools(full_tools),
                response_format=None if not response_format else {"type": "json_object", "schema": response_format},
                temperature=temperature,
                max_tokens=max_tokens,
                stream=False
            )
            self.last_response = chat_completion
            assert not isinstance(chat_completion, Iterator)  # should not fail since stream=False
            return from_chat_completion(chat_completion)
    
    def invoke_tool_calls(self, tool_calls: list[ToolCall], tools: list[Tool]) -> list[ToolResult]:
